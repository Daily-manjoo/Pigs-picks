...
export default function Paginiation({total, page}: Pagination){
    return(
        <div className="py-6 w-full px-10 flex justify-center gap-4 bg-white my-10 flex-wrap text-black">
                    {total <= 10 ? [...Array(total)].map((x, i) => (
                        <Link href={{pathname: "/stores", query: {page: i + 1}}} key={i}>
                            <span className={`px-3 py-2 rounded border shadow-sm bg-white ${(i + 1 === parseInt(page, 10) ? 'text-blue-600 font-bold' : 'text-gray-300' )}`}>
                                {i + 1}
                            </span>
                        </Link> //0부터 시작하니까 1페이지부터 보여주기
                    )) : 
                        <>
                            {parseInt(page) > 1 && (
